...
cron "fetch-upcoming-pass-info" do
  time :daily
  command "/bin/bash -l -c 'cd /www/sandy/current && bin/rails runner -e production '\''Facility.queue_upcoming_passes'\'''"
...
cron 'cleanup-old-passes' do
  time :hourly
  command "/bin/bash -l -c 'cd /www/sandy/current && bin/rails runner -e production '\''Pass.cleanup_old_passes'\'''"
  environment({"PATH" => "$PATH:/opt/sandy/embedded/bin"})
  user 'processing'
end
